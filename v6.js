...
(()=>{
'use strict';
const VERSION='6.4.106-cosmetic-accordions', SNAPSHOT_PREFIX='jm_v6_snapshots_', FEEDBACK_INBOX_KEY='jm_v6_admin_feedback_inbox', FEEDBACK_INBOX_LIMIT=200;
...
function openSettings(){
 ...
 const renderPortraitSetting=(message='')=>{const enabled=state.v6.portraitLockEnabled!==false;portraitButton.textContent=enabled?'Unlock rotation':'Lock portrait';portraitButton.classList.toggle('primary',!enabled);portraitButton.setAttribute('aria-pressed',String(enabled));portraitStatus.textContent=message||(enabled?'Portrait lock is on.':'Rotation is unlocked.');};
 renderPortraitSetting();
 portraitButton.onclick=async()=>{const enabled=state.v6.portraitLockEnabled===false;state.v6.portraitLockEnabled=enabled;save();renderPortraitSetting(enabled?'Trying portrait lock…':'Rotation is unlocked.');const result=await window.setJapaneseMinerPortraitLock?.(enabled);if(!enabled){renderPortraitSetting('Rotation is unlocked.');return;}if(result?.locked){renderPortraitSetting('Portrait is locked.');return;}if(result&&!result.supported){renderPortraitSetting('Portrait preference is on. This device does not offer app-controlled rotation.');return;}renderPortraitSetting(result?.mobile?'Portrait preference is on. If the phone still rotates, use its system rotation control.':'Portrait lock is on for phone play.');};
...
const originalOpenPatreon=openPatreon;openPatreon=function(){originalOpenPatreon();const tierOneDescription=document.querySelector('#v6FeedbackContent .supporter-tier.tier-one p');if(tierOneDescription)tierOneDescription.textContent='Clothing, hats, accessories, rock skins, mine wallpapers, pickaxe skins, full-page wallpapers, character titles, Discord role, behind-the-scenes posts, development updates, poll voting, and optional Supporter credits.';document.getElementById('v6FeedbackContent')?.insertAdjacentHTML('beforeend','<a class="supporter-join-link" href="https://www.patreon.com/cw/Erendragneel/membership" target="_blank" rel="noopener">Join Language Miner on Patreon</a>');window.renderJapaneseMinerPatreonLinking?.(document.getElementById('v6FeedbackContent'));};
